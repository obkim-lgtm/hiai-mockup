...
- **평가 설계 작업 전 → [sessions/2026-09-03.md](sessions/2026-09-03.md) 맨 위 `09-10` 절(저장 선택창 최종, 배포·PRD 반영) + 맨 아래 "2026-09-08 오후" 절과 "▶ 이어받기"** — 작업본 **`../output/evaluation_design_v9.html`**(v8은 회의록·PRD에 공유된 주소라 보존). 설계는 채점기준까지 = 완료, 그 위에 **AI 채점 정확도 높이기 3단계**(① 빈 활동지·문항 확정 ② 채점기준 연결(매트릭스) ③ 모범답안). **v7의 AI 진단·루브릭 수정 제안은 삭제됨**
...
- **⚠ 변경 후 처리는 26-09-08 개발 회의로 확정: 초기화하지 않는다, 지운 것만 지운다** → [sessions/2026-09-03.md](sessions/2026-09-03.md) "2026-09-08 오후" 절. 급간·채점요소 삭제 → 그걸로 매긴 점수만 삭제(학생 `확인 필요`). 설계 문항은 OCR과 연결되지 않음(인식 결과 항상 유지). 채점 후 저장 시 `기존 점수 유지 / 초기화` 선택창. v8의 끊김·초기화·사전 안내 모델은 폐기(그 기록은 09-03 오후~09-08 오전 절, 참고용)
...
- **백오피스 OCR 인식 결과 검수 목업 → ../output/backoffice_ocr_review_v1.html** (목록: 확인 필요 %·의견·P/F 필터 / 상세: 비식별화 원본 ↔ 현재 버전 ↔ 비교 모델 3열 + 과제물 단위 P/F). 기존 `과제물 검수`(빌·전처리) 아래에 신설하는 메뉴 안. 좌측 NB 포함
