...
- **평가 설계 작업 전 → [sessions/2026-09-01.md](sessions/2026-09-01.md)의 "▶ 이어받기"** — 작업본 `../output/evaluation_design_v8.html`. 설계는 채점기준까지 = 완료, 그 위에 **AI 채점 정확도 높이기 3단계**(① 빈 활동지·문항 확정 ② 채점기준 연결(매트릭스) ③ 모범답안). **v7의 AI 진단·루브릭 수정 제안은 삭제됨**
- **채점기준 AI 검토(루브릭이 채점하기 적합한지) 손대기 전 → [sessions/2026-09-02.md](sessions/2026-09-02.md)** — AI팀이 실제 만든 API를 v8 채점기준 섹션에 붙인 것. 판정은 하드코딩이 아니라 입력된 텍스트를 훑는 규칙(`rvScan`)이라 교사가 고쳐 쓰면 따라 바뀐다. `overloaded_element`(섞임) · `ambiguous_boundary`(경계 흐릿) 두 가지
- **⚠ 채점기준 AI 검토는 시안3(배너 인라인)으로 확정 → [sessions/2026-09-03.md](sessions/2026-09-03.md)** — 실제 계약은 `{ grade, comment }` **필드 두 개뿐**(등급 `우수`·`적절`·`보완필요` 3단계 + 글 하나). 채점기준별이 아니라 **전체 루브릭 단위**라 매핑도 카드 표시도 없다. 드로워·별도 화면 없이 **배너 한 자리에서 끝난다**. 시안1(`#case=rv-hit`)·시안2(`#case=rv-page`)는 전제가 달라 접혔으나 **지우지 말 것**(까망이에 `(보류)`로 남김). `S.rvPlan` 1\|2\|3이 가른다
- **⚠ stale(뒤 단계가 낡거나 지워지는 것) 손대기 전 → [sessions/2026-09-03.md](sessions/2026-09-03.md) "오후" 절** — 두 축을 섞지 않는다: *지워지는 것*(채점 결과·인식 결과)은 **저장할 때 모아서 한 번** 묻고, *낡는 것*(연결·모범답안)은 **표시만** 한다(막지 않음). 활동지 교체·문항 재분석 확인창은 `resetWarnHtml()` 하나를 공유하며 **갯수는 세지 않는다**. 까망이 `고친 뒤 처리` 탭에 케이스 8종. **다음 세션이 이어받는 자리**
- **맥이 준 실제 계약 원본** → 슬랙 `#hiai` 첨부 `rubric-ai-review-mockup.html`. ⚠️ 아티팩트 링크(`claude.ai/code/artifact/c6d8f3c8…`)는 캐시 때문에 옛 버전이 뜬다 — **첨부 HTML이 진실**
