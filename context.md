...
## 저장 정책 (채점 vs OCR) — 2026-07-14 확정

> 두 번 뒤집힌 정책. 현행 기준은 Figma `MM23uA7pDEmFeMKGeFVJpB` `1751:4122` 주석("채점은 자동 저장 없이, 기존처럼 저장 버튼 누르면 저장됨. 채점 요소별로 이루 저장X").

| 영역 | 저장 방식 | UI |
|---|---|---|
| **채점** (점수·채점상태·선생님 피드백) | **사용자 저장 (자동 저장 없음)** | 타이틀바 우측 **[저장]** primary 버튼(40px, check 아이콘) + 하단 **[저장 후 다음 학생]**. 요소별 개별 저장 없음 |
| **OCR** (인식 결과 텍스트 편집) | **자동 저장 유지** (blur 시) | "수정하면 자동으로 저장돼요" → "자동 저장됨 · 시각" 필 (`task_ocr_review_v1.html`) |

- 타이틀바 좌측: 드롭다운 옆 **"n명 중 n명 채점 완료"** 진행 표시(초록 check).
- 적용 파일: `scoring_student_v1/v2/v3.html`, `scoring_student_kimmh/kimhi_v1.html`(자동저장 필→저장 버튼 롤백), `scoring_student_elementary_v1.html`(blur 자동저장 토스트 제거, [저장 후 다음]에 점수 미선택 검증 이동).

---

## 작업 이력
- 260714: **저장 정책 확정 반영** — 채점 자동저장 롤백(저장 버튼 방식, Figma `1751:4122`), OCR 자동저장 유지. `scoring_student_v1/v2/v3/kimmh/kimhi` 자동저장 필→[저장] 버튼 + "n명 중 n명 채점 완료" 표시, `elementary` blur 자동저장 토스트 제거.
